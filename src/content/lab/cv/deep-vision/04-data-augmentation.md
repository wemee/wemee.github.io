...
直覺上「資料變多模型該更準」,但你會看到開了增強後**訓練準確率不升反降**,容易以為增強幫倒忙。

> 這正是增強在發揮作用。每個 epoch 看到的都是被裁切、翻轉、調色過的**變體**,題目變難了,訓練準確率當然下降。真正要看的是**測試準確率**——它通常會上升,訓練與測試之間那道縫也會縮小。訓練分數低一點、測試分數高一點,才是「沒在背圖、真的學會」的健康徵兆。
