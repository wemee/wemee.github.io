...
'''
        ...
        md(
            "## 4. 畫學習曲線\n\n"
            "報酬往上爬,代表 agent 在進步。但**先別急著慶祝**——盯著這條曲線看,"
            "它很可能爬到頂之後又崩下來。下一節解釋為什麼。"
        ),
        code(PLOT_SRC),
        code('plot_rewards(rewards_hist, window=20, title="Hand-crafted DQN on CartPole")'),
        md(
            """
## 5. ⚠️ 曲線爬到頂又崩回去?這不是 bug

很多人第一次跑會看到這個形狀:報酬一路爬到接近滿分 500,**然後突然崩回一兩百、卡在那裡**。先說結論:

> 這**不是 overfitting,也不是你寫錯**,而是 DQN 惡名昭彰的不穩定性——**災難性遺忘(catastrophic forgetting)**。它幾乎是手刻 DQN 的招牌曲線。

為什麼會這樣?三個主因,而且都跟我們的程式有關:

1. **經驗回放被「好回合」洗掉多樣性(最大主因)**。`buffer` 上限一萬筆,但 agent 變強後一個回合長達 ~500 步,**兩三個好回合就把整個 buffer 塞滿**——裡面幾乎只剩「平衡得很好」的狀態,早期那些「快跌倒、要搶救」的失敗樣本被擠光了。agent 於是**忘了怎麼從危險姿勢救回來**,一遇到就垮。
2. **Q 值過度估計**。`max` 運算子天生會高估 Q 值(deadly triad:函數逼近 + bootstrapping + off-policy),估計越推越偏。
3. **訓練過頭、沒有早停**。RL 裡「訓練越久 ≠ 越好」,跑過巔峰後策略會漂走。我們固定跑 300 回合、不看表現好壞一路更新,等於眼睜睜看它衝過頭再墜落。

> 注意:epsilon 在約 120 回合就降到 0.05 地板了,所以後段的崩潰**不是探索率突變造成的**,純粹是上面這些內在不穩。

**所以這張圖其實是「對的」**:峰值證明這個迷你 DQN 真的學得會;崩潰證明了為什麼真實系統要加一堆穩定技巧——**eval 早停存檔點**、**Double DQN**(治高估)、**更大/prioritized 的 buffer**(保住失敗樣本)、**gradient clipping**、**target network soft update**。下一課的 stable-baselines3 把這些都內建好了,曲線會穩得多。這正是「手刻懂原理、現成做專案」缺一不可的最佳示範。

## 小結

- **DQN = 用神經網路取代 Q 表**,解決連續/巨大狀態空間。
- 兩個讓訓練穩定的關鍵:**經驗回放** 與 **target network**。
- **曲線爬到頂又崩**是 DQN 的**災難性遺忘**,不是 overfitting、不是寫錯——這是手刻版的常態。
- 親手刻一遍,你就懂了現成函式庫底下到底在做什麼、以及在防什麼。

下一課:同樣的事,用 **stable-baselines3** 一行搞定——它把防崩的技巧都內建好了,並比較手刻與現成的差別。
"""
        ),
    ]
